...
void PROTO_CONVERTER_Test_SendID003Message(void)
{
    ...
    MESSAGE_Init(&id003_request_msg, PROTO_ID003, id003_opcode);
...
void PROTO_CONVERTER_Test_ReceiveID003Response(void)
{
    ...
    MESSAGE_Init(&received_id003_response_msg, PROTO_ID003, id003_response_raw[2]);
...
void PROTO_CONVERTER_Test_SendCCNETResponse(void)
{
    ...
    MESSAGE_Init(&ccnet_response_msg, PROTO_CCNET, ccnet_response_opcode);
...
static void PROTO_CONVERTER_ParseCCNETMessage(const uint8_t* raw_data, uint8_t length, message_t* msg)
{
    ...
    if (length >= 4)
    {
        MESSAGE_Init(msg, PROTO_CCNET, raw_data[3]); /* opcode is at index 3 */
